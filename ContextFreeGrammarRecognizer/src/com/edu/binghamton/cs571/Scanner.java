...
/**
 * Scanner built from a mapping of regex strings to some enum of token-kinds. It
 * regex string is empty, then used for EOF token. If mapped enum is null, then
 * input matching the corresponding regex is ignored.
 * 
 * Quite practical except for following problems:
 * 
 * 1) Difficult to have different scanning rules depending on context (as per
 * the states of lex-like scanners). This can be worked-around by defining
 * pseudo-token-kinds with state arguments (regular tokens would have null state
 * arguments). If such a token is produced, then the scanner would switch tables
 * to that specified by the state.
 * 
 * 2) Matching regex is found using linear search over all patterns. This is a
 * problem with the Java Matcher API and does not seem to have any easy
 * workaround.
 * 
 * 3) Lexemes cannot cross multiple lines. This should have a easy workaround by
 * sucking in entire file into memory (though this will not work for gargantuan
 * files).
 */
class Scanner {
	...
	Scanner(LinkedHashMap<String, Enum> scanMap) {
		this(new BufferedReader(new InputStreamReader(System.in)), new Coords(
				"<stdin>"), scanMap);
	}

	/**
	 * Create a scanner for reader with previous coordinates coords. scanMap is
	 * a map from regex-strings to corresponding token kinds (the token kind is
	 * null if the lexeme should be skipped).
	 */
	...
	Token nextToken() {
		...
		while (token == null) {
			...
			Matcher matcher = _scanMapInfo.pattern.matcher(_line).region(colN,
					_line.length());
	...
	private static ScanMapInfo analyzeScanMap(
			LinkedHashMap<String, Enum> scanMap) {
		Map<String, Enum> nameKinds = new LinkedHashMap();
			...
			if (regex.length() == 0) {
				if (eofKind != null) {
					throw new RuntimeException(
							"multiple empty patterns for EOF token");
				}
				eofKind = e;
				continue;
			}
		...
		return new ScanMapInfo(Pattern.compile(b.toString()), nameKinds,
				eofKind);
	}
	...
	private static class ScanMapInfo {
		...
		ScanMapInfo(Pattern pattern, Map<String, Enum> nameKinds, Enum eofKind) {
			...
			this.nameKinds = nameKinds;
			this.eofKind = eofKind;
		}
	}

	private static enum T {
		EOF, ID, INT, STRING, MISC
	};
	...
	public static void main(String[] args) {
		...
		String test = "id_123 \"some string\", #21 34\n"
